...
export default function CoreArtPlayerGame({
  source,
  poster,
  ...
  const handleRetry = () => {
    ...
    const delay = RETRY_DELAY_MS * Math.pow(2, nextRetry - 1);
  ...
  if (!url) {
    return (
      <div className={cn(`relative w-full h-[250px] md:h-full bg-app-background rounded overflow-hidden`, className)}>
        {poster && (
          <Image
            src={poster}
            alt='poster match'
